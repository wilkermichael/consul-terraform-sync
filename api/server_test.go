...
func TestRequest(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			c, err := NewClient(&ClientConfig{}, hc)
			require.NoError(t, err)
...
func TestStatus(t *testing.T) {
	...
	c, err := NewClient(&ClientConfig{Addr: fmt.Sprintf("http://localhost:%d", port)}, nil)
	require.NoError(t, err)
...
func TestWaitForAPI(t *testing.T) {
	...
	t.Run("timeout", func(t *testing.T) {
		cts, err := NewClient(&ClientConfig{}, nil)
		require.NoError(t, err)
	...
	t.Run("available", func(t *testing.T) {
		...
		cts, err := NewClient(&ClientConfig{Addr: fmt.Sprintf("http://localhost:%d", port)}, nil)
		require.NoError(t, err)
